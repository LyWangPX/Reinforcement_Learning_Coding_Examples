...
"""
This is a vanilla Deuling Double DQN with PER (softmax absolute delta)
"""
...
class Q_network(torch.nn.Module):
    def __init__(self, n_action=2):
        ...
        self.priority_buffer = deque(maxlen=8192)
    ...
    def bufferin(self, tuple_info):
        ...
        state, action, reward, next_S, done, priority = tuple_info
    ...
    def sample(self, size=64):
        with torch.no_grad():
            prob = np.array(F.softmax(torch.stack(list(self.priority_buffer))).view(-1))
            prob /= prob.sum()
            sample_indices = np.random.choice(range(len(self.a_buffer)), size=64, p=prob, replace=False)
        ...
        return s_sample, a_sample, r_sample, next_s_sample, done_sample, sample_indices
...
def main():
    ...
    Q_target = Q_network()
    optimizer = torch.optim.Adam(Q_target.parameters(), lr=0.0003)
    Q_copy = Q_network()
    ...
    for episode in range(10000):
        ...
        for step in range(200):
            with torch.no_grad():
                ...
                # PER: Calculate delta of this tuple
                Q = Q_list[0][action]
                Q_prime = Q_copy.forward(next_state)
                next_action = np.argmax(Q_prime.detach())
                delta = abs(Q - reward + gamma * Q_prime[0][next_action])
                delta = delta.view(1)
                tuple_info = (state, action, torch.Tensor([reward]), next_state, not done, delta)
                Q_target.bufferin(tuple_info)
            # Learning Part
            if len(Q_target.a_buffer) > 64:
                s_sample, a_sample, r_sample, next_s_sample, done_sample, ids = Q_target.sample()
                ...
                deltas = abs(Q - y.detach())
                for delta, id in zip(deltas, ids):
                    Q_target.priority_buffer[id] = delta.view(-1)
    ...
    plt.title(f'Deuling DDQN on CartPole-v0 with PER', fontsize='xx-large')
